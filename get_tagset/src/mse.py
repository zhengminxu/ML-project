...
class process_features:
	...
	def gen_train(self, x_train_c, val):
		...
		for x in x_train:
			if x in xx:
				y_train += [xx[x] / self.maxtag if xx[x] <= self.maxtag else 1]
			else:
				y_train += [0]
	...
	def get_train(self):
		if os.path.isfile("../feat/feats.npy"):
			...
			x_train, x_val, y_train, y_val, tags = np.load("../feat/feats.npy")
		else:
			(x_train_c, x_val_c) = self.get_content(self.feat_path)
			x_train, y_train = self.gen_train(x_train_c, False)
			x_val, y_val, tags = self.gen_train(x_val_c, True)
			#rearranging dims
			...
			print "Saving feats"
			np.save("../feat/feats", (x_train, x_val, y_train, y_val, tags))
		return x_train, x_val, y_train, y_val, tags
...
#		data = [lmtzr.lemmatize(x, pos='v') for x in data if cond(x)] #not sure
	...
	def rearrange(self, x, y):
		v1_trunc = x[:y.shape[0]].transpose()
		v1 = x.transpose()
		v2 = y.transpose()
		def swap(v, a, b):
			temp = np.copy(v[a])
		...
		for idx, x in enumerate(v2):
			_mse = np.mean((v1_trunc[idx:] - x) ** 2, axis=1)
			_result = idx + np.argmin(_mse)
			swap(v1_trunc, idx, _result)
			swap(v1, idx, _result)
		return v1.transpose()

feat = process_features(sys.argv[1:])
x_train, x_val, y_train, y_val, tags = feat.get_train()
tags = np.array(tags)

model = Sequential()
model.add(Dense(256, input_shape=(embed_SIZE,), activation='relu', W_regularizer=l1(0.01)))
model.add(Dropout(0.5))
model.add(Dense(128, activation='relu', W_regularizer=l1(0.01)))
