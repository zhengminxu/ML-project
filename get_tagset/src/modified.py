...
class process_features:
	...
	def gen_train(self, x_train_c, val):
		...
		if val :
			...
			x_train = np.concatenate((x_train, np.zeros(self.doclen - x_train.shape[0], x_train.shape[1])), axis=1)
		print x_train
		gc.collect()
		print "Doing LSA"
		print "SVD...."
...
#		data = [lmtzr.lemmatize(x, pos='v') for x in data if cond(x)] #not sure
	...
	def get_content(self, path):
		...
		#self.vectorizer.fit(all_datas)
		#self.val_vectorizer = TfidfVectorizer(use_idf=False)
		#self.val_vectorizer.fit(val_data)
